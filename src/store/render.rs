...
pub fn render_replay(record: &RunRecord) -> String {
    ...
    if let Some(profile) = &record.cli.instruction_task_profile {
        out.push_str(&format!("instruction_task_profile: {}\n", profile));
    }
    if let Some(task_kind) = &record.cli.instruction_task_profile_task_kind {
        out.push_str(&format!(
...
mod tests {
    ...
    #[test]
    fn render_includes_profile_name_and_canonical_task_kind_when_they_differ() {
        let rendered = render_replay(&crate::store::RunRecord {
            mode: "single".to_string(),
            planner: None,
            worker: None,
            ...
            cli: crate::store::RunCliConfig {
                ...
                instructions_config_hash_hex: None,
                instruction_model_profile: None,
                instruction_task_profile: Some("coding_orchestrator_v1".to_string()),
                instruction_task_profile_task_kind: Some("coding".to_string()),
                instruction_message_count: 0,
                project_guidance_hash_hex: None,
                project_guidance_sources: Vec::new(),
                project_guidance_truncated: false,
        ...
        assert!(rendered.contains("instruction_task_profile: coding_orchestrator_v1"));
        assert!(rendered.contains("instruction_task_profile_task_kind: coding"));
    }
}
